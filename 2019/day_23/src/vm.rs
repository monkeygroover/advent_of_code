...
impl VM {
    pub fn new(address: i64, mut initial_memory: Vec<i64>) -> VM {
    ...
    fn read(&mut self, modes: Vec<i64>) -> State {
        ...
        trace!("{} read pending for [{}] {:?}", self.address, param, modes);
    ...
    fn set_ix(&mut self, modes: Vec<i64>) -> State {
        ...
        self.ix += a;
        debug!("{} ix {} -> {}", self.address, orig_base, self.ix);
        self.pc = self.pc + 2;
        State::Continue
    }
}
